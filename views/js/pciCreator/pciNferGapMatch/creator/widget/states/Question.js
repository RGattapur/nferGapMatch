...
define([
  ...
  // 'tpl!pciNferGapMatch/creator/tpl/propertiesForm',
  // 'tpl!pciNferGapMatch/creator/tpl/gap-create',
  // 'tpl!pciNferGapMatch/creator/tpl/htmlEditorTrigger'
], function (
  $,
  _,
  __,
  ...
  var GapMatchInteractionStateQuestion = stateFactory.extend(
    Question,
    function () {
      this.buildEditor();
  ...
  GapMatchInteractionStateQuestion.prototype.initGapCreator = function initGapCreator() {
    var self = this,
      ...
      $editable = interactionWidget.$container.find(
        ".qti-flow-container [data-html-editable]"
      ),
    ...
    $newGapBtn.on("mousedown.gapcreator", function () {
      ...
      if (wrapper.wrapWith($newGap)) {
        ...
        feedback().error(
          __(
            "Cannot create gap from this selection. Please check that you do not have partially selected elements."
          )
        );
      }
    });
  ...
  GapMatchInteractionStateQuestion.prototype.createChoiceFromSelection = function createChoiceFromSelection(
    ...
    var interactionWidget = this.widget,
      ...
      $addChoiceBtn = interactionWidget.$container.find(
        ".choice-area .add-option"
      ),
      $nestedWidgets,
      nestedElts,
      newChoiceElt,
    ...
    $nestedWidgets = $newChoiceContent.find(
  ...
  GapMatchInteractionStateQuestion.prototype.syncCardinality = function () {
    ...
    var updateCardinality = function (data) {
      ...
      if (choice.qtiClass === "gap") {
        cardinality =
          _.size(interaction.getGaps()) === 1 ? "single" : "multiple";
  ...
  GapMatchInteractionStateQuestion.prototype.preventSingleChoiceDeletion = function () {
    ...
    var _toggleDeleteButtonVisibility = function () {
      var choiceCount = 0,
        $deleteButtons = $container.find(
          ".choice-area .qti-choice [data-role=delete]"
        );
