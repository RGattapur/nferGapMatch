...
define([
  ...
  function initMapState() {
    ...
    var currentResponses =
      _.size(response.getMapEntries()) === 0
        ? corrects
        : _.keys(response.getMapEntries());
    ...
    instructionMgr.appendInstruction(
      interaction,
      __(
        "Please fill the gap with the texts below, then edit the score for each pair."
      )
    );
    ...
    widget.$container.on("responseChange.qti-widget", function (e, data) {
      ...
      if (data && data.response && data.response[type]) {
        pairs = _.invoke(
          data.response[type].directedPair,
        ...
        _(pairs)
          .difference(entries)
          .forEach(
            interaction.pairScoringForm.addPair,
            interaction.pairScoringForm
          );
      }
  ...
  function updateForm(widget, entries) {
    ...
    var formatedChoices = _.transform(
    ...
    if (entries) {
      options.entries = _.transform(
        entries,
        function (result, value) {
          result[value] =
            mapEntries[value] !== undefined
              ? mapEntries[value]
              : response.mappingAttributes.defaultValue;
        },
        {}
      );
    }
